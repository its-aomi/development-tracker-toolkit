...
This development tracker does actually fit the second half of this definition: it is set up to not require a backend or server. If our dataset were much larger, we wouldn't want to do that pre-generation of pages, instead using a backend which would fetch data on-demand from a database and only build a page when the end user requests that page. 
