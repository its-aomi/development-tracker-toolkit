...
The projects table is the first and main table you’ll be using, shown by default in grid view. It is where you add information about real estate developments. You can use different views or interfaces to add this information. Below, we walk through all the data you’ll enter for each record and how it can or should be formatted, going in sequence through the fields as they are displayed in the grid view.
...
* Choose a consistent naming convention; we used the name from the project field without articles.
* Generally, you don’t want to change these after publishing, since it will change the live URL of the record – if someone’s already linked to a record from elsewhere, that link will be broken. 
...
This is an internally used field with the time a record was created automatically generated by Airtable. 
...
This is an internally used field with the time a record was last modified automatically generated by Airtable. 
...
* **Error: No address:** These records need addresses. 
* **Error: Image, no caption:** Add a caption to these records.
...
* **Dashboard – Record stats:** Shows record and data entry status with stats like the total number of records and charts like the number of records assigned to each collaborator. 
...
Again, you will need to configure any automations for yourself – here’s [more info on how to do it](https://support.airtable.com/hc/en-us/articles/360050974153-Automations-overview). We use automations to send a Slack message when records are marked ready for review and published, so the editor and publisher knows when they have tasks to complete.  
