...
* Go to "Policies", under "Configuration", and create a "New policy" on the `report-photos` bucket.
...
* Navigate to your Supabase project settings, which is the gear icon on the bottom of the left-hand sidebar.
...
* Copy the URL of the Supabase project, including the `.supabase.co` suffix, under "Configuration", as the `NEXT_PUBLIC_SUPABASE_URL` in `.env.development.local`.
